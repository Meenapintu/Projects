...
void *Alloc_Pageable_Page(pte_t * entry, ulong_t vaddr) {
    ...
    paddr = Alloc_Page();
    // Print("Alooc Page mapping %8lx -> %8lx\n", paddr, vaddr);
    if (paddr != 0) {
        ...
        if (page->flags & PAGE_ALLOCATED) {
            ...
            ulong_t entry_addr = (ulong_t) page->entry;
            if(entry_addr >= (ulong_t) kernelDir && 
               entry_addr < (ulong_t)kernelDir + 512 * sizeof(int)) {
                if(CURRENT_THREAD->userContext) {
                    memcpy(CURRENT_THREAD->userContext->pageDir, 
                            kernelDir, 512 * sizeof(int));
                }
            }
        } else {
